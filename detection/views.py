...
def disease_list_view(request):
    ...
    for plant in Plant.objects.all().prefetch_related('diseases'):
        ...
        plant_name = plant.get_translated_name('tl') if current_language == 'tl' else plant.name
        plant_data = {'name': plant_name, 'original_name': plant.name, 'diseases': []}
...
def handle_test_mode(request, plant_type_str, uploaded_image):
    ...
    if db_plant_name:
        try:
            ...
            try:
                disease_obj = Disease.objects.get(name__iexact=prediction_label, plants=plant_obj)
            except: pass
        except: pass
...
def make_real_prediction(plant_type_str, full_file_path):
    ...
    if plant_type_str == 'eggplant' and eggplant_model:
        ...
        if processed_image is not None:
            prediction_label, confidence = predict_pytorch(eggplant_model, processed_image, eggplant_class_names)
            
    elif plant_type_str == 'chili' and chili_model:
        chili_class_names = ["Damaged", "Dried", "Old", "Ripe", "Unripe"]
        processed_image = preprocess_image_torch(full_file_path)
        if processed_image is not None:
            prediction_label, confidence = predict_pytorch(chili_model, processed_image, chili_class_names)
        ...
        if tomato_leaf_model:
            ...
            if processed_image is not None:
                ...
                if leaf_prediction and leaf_prediction != "Healthy Tomato Leaves":
                    prediction_label, confidence = leaf_prediction, leaf_conf
                elif tomato_vege_model:
                    ...
                    if vege_processed_image is not None:
                        prediction_label, confidence = predict_keras(tomato_vege_model, vege_processed_image, tomato_vege_class_names)
                else:
                    prediction_label, confidence = "Tomato leaf model detected healthy", leaf_conf
                    
    return prediction_label or "No disease detected", confidence or 0.0

def get_disease_info(prediction_label, plant_type_str, current_language):
    plant_mapping = {'chili': 'Chili Pepper', 'eggplant': 'Eggplant', 'tomato': 'Tomato'}
    db_plant_name = plant_mapping.get(plant_type_str)
    plant_obj = None
    disease_obj = None
    ...
    if db_plant_name:
        try:
            plant_obj = Plant.objects.get(name=db_plant_name)
            disease_obj = Disease.objects.filter(name__iexact=prediction_label, plants=plant_obj).first()
        except: pass
    ...
    if disease_obj:
        if current_language == 'tl':
            final_disease_name = disease_obj.get_translated_name('tl') or disease_obj.name
    ...
    return final_disease_name, disease_description, disease_symptoms, disease_prevention, plant_obj, disease_obj if is_disease else None, is_disease
        ...
        try:
            if TEST_MODE:
                ...
                if not uploaded_image or not plant_type_str:
                    return JsonResponse({'error': 'Missing image or plant type'}, status=400)
                return handle_test_mode(request, plant_type_str, uploaded_image)

            if not models_loaded_successfully:
                return JsonResponse({'error': 'No machine learning models are available'}, status=503)
            ...
            plant_model_available = (
                (plant_type_str == 'eggplant' and loaded_models['eggplant']) or
                (plant_type_str == 'chili' and loaded_models['chili']) or
                (plant_type_str == 'tomato' and (loaded_models['tomato_leaf'] or loaded_models['tomato_vege']))
            )
            if not plant_model_available:
                return JsonResponse({'error': f'Model for {plant_type_str} is not available'}, status=503)
            ...
            prediction_label, confidence = make_real_prediction(plant_type_str, full_file_path)

            if 'saved_file_name' in locals() and default_storage.exists(saved_file_name):
                default_storage.delete(saved_file_name)

            current_language = get_current_language(request)
            final_disease_name, disease_description, disease_symptoms, disease_prevention, plant_obj, disease_obj, is_disease = get_disease_info(
